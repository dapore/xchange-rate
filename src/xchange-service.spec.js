...
import getXchangeService from '../src/xchange-service';
...
test('MakeRequest can throw', async t => {
  ...
  const xchangeService = new Client();
  try {
    const rate = await xchangeService.makeGetRequest(`${t.context.api}/api/error`)
  } catch (error) {
...
test('Get rateMetaData', async t => {
  const meta = `meta`
  const nockie = nock(`https://www.google.com/finance`)
